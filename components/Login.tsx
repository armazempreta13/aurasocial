...
export function Login() {
  ...
  const handleAuthError = (err: any) => {
    if (err.code === 'auth/unauthorized-domain') {
      ...
      setError(t('login.not_allowed', 'Este método de login não está ativado no Console do Firebase.'));
    } else if (err.code === 'auth/account-exists-with-different-credential') {
      setError(t('login.account_exists', 'Já existe uma conta com este e-mail vinculada a outro provedor de login.'));
    } else if (err.code === 'auth/popup-closed-by-user') {
      // Ignorar silenciamento se o usuário fechar o popup
    } else {
      setError(err.message);
    }
  };
        ...
        <div className="mt-auto w-full pt-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-6">
            ...
            <button
              ...
              disabled={loading}
              type="button"
              className="w-full bg-[#24292e] hover:bg-[#1b1f23] text-white font-semibold py-3.5 rounded-2xl transition-all flex items-center justify-center gap-2.5 disabled:opacity-70 disabled:cursor-not-allowed active:scale-[0.98] text-[14px]"
            >
